...
contract SimpleDividendToken {
    ...
    mapping(address => uint256) dividendBalanceOf; // amount of ether credited to each account but not yet transferred.

    uint256 public dividendPerToken; // total amount of ether per token that has been deposited.

    mapping(address => uint256) dividendCreditedTo; //total amount of ether per token that has been previously credited to the account 
